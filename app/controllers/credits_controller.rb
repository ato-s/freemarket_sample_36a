...
include Payjp_process
  ...
  def create
    @credit = Credit.create(credit_params)
      if @credit.save
        redirect_to new_transactions_credit_path
      else
        render :new
      end
  end
  ...
  def credit_params
    params.require(:credit).permit(:credit_number,:limit_month,:limit_year,:security_code).merge(customer_id: create_customer.id)
